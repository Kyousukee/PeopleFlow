...
using PeopleFlow.Formularios.Deducciones;
using System;
...
namespace PeopleFlow.Formularios
{
    public partial class FrmDeducciones : Form
    {
        ...
        private void btnIngresar_Click(object sender, EventArgs e)
        {
            try
            {
                DTO.Deducciones datos = new DTO.Deducciones();
                if (Validar()) return;
                datos.IdDeduccion = 0;
                ...
                DTO.Deducciones response = new BO.BODeduccion().registrarBeneficio(datos);
                if (response.IdDeduccion > 0)
                {
                    Mensaje("Deduccion registrada correctamente.");
                    cargarDeduccion(response);
                }
                else
                {
                    Mensaje("Error al registrar la Deduccion.");
                }
            }
            catch (Exception ex)
            {
                logger.Error(ex, $"\"Error al registrar la Deduccion: {this.Name}");
                Mensaje("Error: " + ex.Message);
            }
        }

        private void cargarDeduccion(DTO.Deducciones bonificaciones)
        {
            try
            {
                txtID.Text = bonificaciones.IdDeduccion.ToString();
            ...
            {
                logger.Error(ex, $"\"Error al cargar Deduccion en formulario: {this.Name}");
                Mensaje("Error al cargar Deduccion en formulario: " + ex.Message);
            }
        }
        ...
        private bool Validar()
        {
            ...
            {
                errores.Add("El valor ingresado no es un número válido.");
            }
        ...
        private void btnActualizar_Click(object sender, EventArgs e)
        {
            try
            {
                DTO.Deducciones datos = new DTO.Deducciones();
                if (Validar()) return;
                datos.IdDeduccion = Convert.ToInt32(txtID.Text);
                datos.Descripcion = txtNombre.Text;
                datos.IdEmpresa = Sesion.UsuarioActual.IdEmpresa;
                datos.Monto = Convert.ToDecimal(txtvalor.Text);
                datos.FechaAplicacion = dttFAsignacion.Value;
            ...
            {
                logger.Error(ex, $"\"Error al editar la Deduccion: {this.Name}");
                Mensaje("Error: " + ex.Message);
            }
        }
        ...
        private void btnListadoTrabajadores_Click(object sender, EventArgs e)
        {
            try
            {
                FrmListadoDeduccion frmShow = new FrmListadoDeduccion();
                frmShow.ShowDialog();
                if (frmShow.selectDeduccion.IdDeduccion != 0)
                {
                    cargarDeduccion(frmShow.selectDeduccion);
                }

            }
            catch (Exception ex)
            {
                logger.Error(ex, $"\"Error al cargar Listado de deducciones en formulario: {this.Name}");
                Mensaje("Error al cargar Listado de deducciones en formulario: " + ex.Message);
            }
        }
